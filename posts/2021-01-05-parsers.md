...
The `Alternative` class is similar to `Monoid` -  `empty` is an identity element, such that 
...
and the stickbug operator `<|>` is an associative binary function between two `Alternative`s:

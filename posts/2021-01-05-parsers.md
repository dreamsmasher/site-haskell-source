...
Let's take a step back and write a formal grammar for this problem.
...
Now that we have a rudimentary parser, we can write some parsing functions. We only really need 3 simple primitives:
...
Now we have everything we need to parse a simple atom (again). `Control.Applicative` exports some really useful functions for `Alternative` types, like `optional`, `some`, and `many` that makes our lives a LOT easier. `Alternative` is extremely similar to `Monoid` -  `empty` is an identity element, such that 
...
num :: Parse Int
num = read <$> some digit -- some denotes one or more, so we don't *have* to use readMaybe
...
```haskell
>>> runParse element "MgOH2"
Just ("Mg", "H2O4")

>>> runParse atoms "H2O2"
Just (("H", 2), "O2")

>>> runParse element "Hi2You!!!"
Just (("Hi", 2), "You!!!")

>>> runParse (many element) "HaHaUNLeSS10"
...
`<*` is like the flipped, `Applicative` version of `>>`. It runs both actions, ignoring the second's result but allowing its effects to propagate. We're just about ready to implement our grammar for real.

```haskell
funcGroup :: Parse [Atom]
funcGroup = do
  elements <- concat <$> bracket (some molec) -- wrap the act of parsing one or more 
  n <- num <|> pure 1
  (pure . map ((n *) <$>)) es
```
```haskell

...
It turns out that `tsort` isn't used in `ld` anymore (`grep`ped for it just now), but it's a cool utility that can come in handy. Don't listen to [O'Reilly](https://docstore.mik.ua/orelly/unix/unixnut/c02_222.htm). Next time you run into a problem where you have to consider a set of objects with relationships, see if you can reduce it to graph traversal. This is usually my go-to intuition when I'm doing Leetcode problems, mainly because the site is biased towards graph-type questions anyways. What are some other domains where this is applicable?
